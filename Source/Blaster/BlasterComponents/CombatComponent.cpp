	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			else
			{
				...
				CrosshairShootingFactor = 0.0f; // reset crosshair shooting factor
			}
...
{
	if (Character == nullptr || CarriedAmmo <= 0)
	{
		return;
	}
...
{
	return (EquippedWeapon != nullptr && bCanFire && !EquippedWeapon->IsAmmoEmpty());
}
